...
mGENE$Context[mGENE$Context=="OpenSea"] <- "Ocean"

ANNOT_COLORS <- list(
  Cluster = c(L="lightgray", R="black"),
  RPMM = c(LL="darkgreen", LR="lightgreen", RL="magenta", RR="purple"),
  Context = c(Island="black", Shore="darkgray", Shelf="lightgray", Ocean="lightblue")
)
...
## Load RPMM results w/ sample order:
...
cumsum(table(cls_tcga$RPMM)[1:3]) #32  61 128 
cumsum(table(cls_dkfz$RPMM)[1:3]) #63 109 147

wrapper_hm(tcgaGENE, c(32, 61, 128), cls_tcga[,c("RPMM","Cluster")])
wrapper_hm(dkfzGENE, c(63, 109, 147), cls_dkfz[,c("RPMM","Cluster")])
